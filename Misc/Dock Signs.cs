...
public void DisplayDockSign()
{
    ...
    IMyTextSurface dock1Screen = GridTerminalSystem.GetBlockWithName("Dock 1 Screen") as IMyTextSurface;
    IMyTextSurface dock2Screen = GridTerminalSystem.GetBlockWithName("Dock 2 Screen") as IMyTextSurface;

    if (dock1Screen == null)
    {
        Echo("No LCD found named: Dock 1 Screen");
        return;
    }

    if (dock2Screen == null)
    {
        Echo("No LCD found named: Dock 2 Screen");
        return;
    }

    dock1Screen.FontSize = 5f;
    dock1Screen.WriteText("Dock 1");

    dock2Screen.FontSize = 5f;
    dock2Screen.WriteText("Dock 2");
}